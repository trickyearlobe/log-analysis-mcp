...
These are the shared Go types used across all tools and parsers in the log-analysis-mcp server.

## Appendix A: Shared Go Types
...
type ParsedLogEntry struct {
    ...
    LineCount   int                    `json:"line_count,omitempty"`   // > 1 for multiline entries
    Timestamp   *string                `json:"timestamp"`             // ISO 8601 or nil if unparseable
    Level       *LogLevel              `json:"level"`                 // nil if not detected
    Source      *string                `json:"source"`                // nil if not detected
...
type Anomaly struct {
    Type          string                 `json:"type"`           // "error_spike", "new_error_type", "gap", "rate_change"
