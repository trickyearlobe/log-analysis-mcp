...
func TestListToolsReturnsAll15(t *testing.T) {
    ...
    expected := map[string]bool{
        ...
        "log_parse":           false,
        "log_filter":          false,
        "log_extract_errors":       false,
        "log_summarize":       false,
        "log_detect_anomalies":     false,
        "log_timeline":         false,
        "log_correlate":       false,
        "log_decompress":      false,
        "log_diff":            false,
        "log_run_remote_command":   false,
    ...
    if len(resp.Tools) != 19 {
        t.Errorf("expected 19 tools, got %d", len(resp.Tools))
    }
...
func TestReadLogsRoundTrip(t *testing.T) {
    ...
    out := callTool[readLogsResult](t, session, "log_read", map[string]any{
        "path": path,
    })
...
func TestTailLogsRoundTrip(t *testing.T) {
    ...
    out := callTool[tailLogsResult](t, session, "log_tail", map[string]any{
        "path":      path,
        "num_lines": 3,
    })
...
func TestSearchLogsRoundTrip(t *testing.T) {
    ...
    out := callTool[searchLogsResult](t, session, "log_search", map[string]any{
        "path":    path,
        "pattern": "ERROR",
    })
...
func TestParseLogsAutoDetectJSON(t *testing.T) {
    ...
    out := callTool[parseLogsResult](t, session, "log_parse", map[string]any{
        "path":        path,
        "format_hint": "auto",
    })
...
func TestFilterLogsByLevel(t *testing.T) {
    ...
    out := callTool[filterLogsResult](t, session, "log_filter", map[string]any{
        "path":  path,
        "level": []string{"ERROR"},
    })
...
func TestExtractErrorsClustering(t *testing.T) {
    ...
    out := callTool[extractErrorsResult](t, session, "log_extract_errors", map[string]any{
        "path": path,
    })
...
func TestSummarizeLogsStatistics(t *testing.T) {
    ...
    out := callTool[summarizeLogsResult](t, session, "log_summarize", map[string]any{
        "path": path,
    })
...
func TestDetectAnomaliesErrorSpike(t *testing.T) {
    ...
    out := callTool[detectAnomaliesResult](t, session, "log_detect_anomalies", map[string]any{
        "path":        path,
        "sensitivity": "high",
    })
...
func TestTimelineEventClassification(t *testing.T) {
    ...
    out := callTool[timelineResult](t, session, "log_timeline", map[string]any{
        "path": path,
    })
...
func TestCorrelateLogsCrossFile(t *testing.T) {
    ...
    out := callTool[correlateLogsResult](t, session, "log_correlate", map[string]any{
...
func TestToolErrorPropagation(t *testing.T) {
    ...
    t.Run("nonexistent file", func(t *testing.T) {
        errText := callToolExpectError(t, session, "log_read", map[string]any{
    ...
    t.Run("nonexistent file via tail", func(t *testing.T) {
        errText := callToolExpectError(t, session, "log_tail", map[string]any{
            "path": "/no/such/file.log",
        })
        if !strings.Contains(errText, "FILE_NOT_FOUND") {
            t.Errorf("expected error to contain 'FILE_NOT_FOUND', got: %s", errText)
    ...
    t.Run("nonexistent file via search", func(t *testing.T) {
        errText := callToolExpectError(t, session, "log_search", map[string]any{
            "path":    "/no/such/file.log",
            "pattern": "test",
        })
        if !strings.Contains(errText, "FILE_NOT_FOUND") {
...
func TestBinaryFileRejection(t *testing.T) {
    ...
    for _, toolName := range tools {
        t.Run(toolName, func(t *testing.T) {
            ...
            // log_search and log_parse require extra args, but they aren't
...
func TestDecompressFileRoundTrip(t *testing.T) {
    ...
    out := callTool[decompressFileResult](t, session, "log_decompress", map[string]any{
        "path": path,
    })
    ...
    readOut := callTool[readLogsResult](t, session, "log_read", map[string]any{
        "path": out.TempPath,
    })
    if len(readOut.Lines) != len(lines) {
        t.Fatalf("log_read on temp file: expected %d lines, got %d", len(lines), len(readOut.Lines))
    }

    // Verify log_tail also works on the temp file with O(N) seek
    tailOut := callTool[tailLogsResult](t, session, "log_tail", map[string]any{
        "path":      out.TempPath,
        "num_lines": 2,
    })
    if len(tailOut.Lines) != 2 {
        t.Fatalf("log_tail on temp file: expected 2 lines, got %d", len(tailOut.Lines))
    }
}
...
func TestDecompressFileNotCompressedError(t *testing.T) {
    ...
    errText := callToolExpectError(t, session, "log_decompress", map[string]any{
        "path": path,
    })
...
func TestDecompressFileThenMultiTool(t *testing.T) {
    ...
    // Step 1: Decompress
    decomp := callTool[decompressFileResult](t, session, "log_decompress", map[string]any{
        "path": path,
    })
    tempPath := decomp.TempPath

    // Step 2: log_summarize on the temp file
    ...
    summary := callTool[summarizeResult](t, session, "log_summarize", map[string]any{
        "path": tempPath,
    })
    ...
    search := callTool[searchLogsResult](t, session, "log_search", map[string]any{
        "path":    tempPath,
        "pattern": "ERROR",
    })
    ...
    tail := callTool[tailLogsResult](t, session, "log_tail", map[string]any{
        "path":      tempPath,
        "num_lines": 5,
    })
    if len(tail.Lines) != 5 {
...
func TestDiffLogsFileVsFile(t *testing.T) {
    ...
    out := callTool[diffLogsResult](t, session, "log_diff", map[string]any{
        "base_path":   basePath,
...
func TestDiffLogsErrorMissingFile(t *testing.T) {
    ...
    errText := callToolExpectError(t, session, "log_diff", map[string]any{
