...
3. **Timeline Analysis**: Use the log_timeline tool to build a timeline of error events%s. Identify when errors started, whether they correlate with other events (deployments, restarts), and if there are patterns in timing.
...
func handleGenerateReport(_ context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
    ...
    if comparisonPath != "" {
        comparisonStep = fmt.Sprintf(`5. **Comparison Analysis**: Use the log_diff tool to compare %q (target) against %q (baseline). Report new errors, resolved errors, rate changes, source changes, and throughput shifts.`, logPath, comparisonPath)
    } else {
...
6. **Timeline**: Use the log_timeline tool to build a chronological sequence of significant events:
...
func handleInvestigateRemote(_ context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
    ...
    if logPaths != "" {
        ...
        discoveryStep = "1. **Discovery**: Use log_discover_remote to find available log files and journal units on all target hosts. Review the results and select the most relevant logs for the investigation."
    }
    ...
    if multiHost {
        correlationStep = "7. **Cross-Host Correlation**: Use log_correlate across gathered files from different hosts to find events that span multiple systems (shared request IDs, trace IDs, or timestamps)."
    } else {
    ...
    var comparisonStep string
    if multiHost {
        comparisonStep = "8. **Cross-Host Comparison**: Use log_diff to compare log files between hosts to identify divergent behaviour — errors on one host but not another, different error rates, etc."
    } else {
