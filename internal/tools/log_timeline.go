...
// RunTimeline builds a chronological event log_timeline from a log file.
func RunTimeline(input TimelineInput) (TimelineOutput, error) {
    ...
    if err := CheckFileAccess(input.Path); err != nil {
        return TimelineOutput{}, fmt.Errorf("log_timeline: %w", err)
    }
    ...
    for {
        ...
        if readErr != nil {
            return TimelineOutput{}, fmt.Errorf("log_timeline: read at line %d: %w", startLine, readErr)
        }
